import React from "react";
import { useField } from "formik";

const PhotoFields = ({
  acceptedFileTypes,
  label,
  onChange,
  value,
  onClick,
  name,
}) => {
  const [field, meta] = useField(name);

  return (
    <>
      <label
        className="block text-[15px] pt-2 font-medium text-[#001e44]"
      >
        {label}
      </label>
      
      <hr/>
      <div className=" items-center justify-center mt- w-[270px] md:w-[210px] lg:w-[270px]">
        <label
          htmlFor="dropzone-file"
          className="flex flex-col items-center justify-center w-full h-20 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
        >
          <input
            id="dropzone-file"
            type="file"
            accept={acceptedFileTypes}
            className="hidden"
            {...field}
            onChange={onChange}
          />
   <>
              <svg
                className="w-8 h-14 mb-4 text-gray-500 dark:text-gray-400"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 20 16"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"
                />
              </svg>
              <p className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                <span className="font-semibold">Click to upload</span>
                or drag and drop
              </p>
            </>
        </label>
        {meta.touched && meta.error && (
          <div className="text-red-500 text-sm mt-1">{meta.error}</div>
        )}
      </div>
    </>
  );
};

export default PhotoFields;